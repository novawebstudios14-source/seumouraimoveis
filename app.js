...
function filter(){const purpose=$('#purpose').value,location=$('#location').value,type=$('#type').value;const items=[...document.querySelectorAll('.property-item')];const matches=(item,key,value)=>value==='all'||item.dataset[key].split(' ').includes(value);let visible=0;items.forEach(item=>{const match=matches(item,'purpose',purpose)&&matches(item,'location',location)&&matches(item,'type',type);item.hidden=!match;if(match)visible+=1;});$('#empty').hidden=visible>0;$('#result-count').textContent=visible===0?'Nenhum imóvel para estes filtros':`${visible} ${visible===1?'imóvel encontrado':'imóveis encontrados'}`;$('#clear').hidden=purpose==='all'&&location==='all'&&type==='all';}
